...
object AdvancedBackPressure extends App {
  implicit val actorSystem = ActorSystem("AdvancedBackPressure")
  implicit val materializer = ActorMaterializer()

  // control backpressure
  val controlledFlow = Flow[Int].map(_ * 2).buffer(10, OverflowStrategy.dropTail)
  ...
  val oncallEngineer = "daniel@rockthejvm.com" //fast service for fetching oncall emails
  ...
  // we have decoupled the upstream rate from the downstream rate using conflate which never backpressures
  ...
  // extrapolate creates the iterator when there is unmet demand
  // expand creates the iterator at all times
